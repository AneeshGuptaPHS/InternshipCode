...
#Opens the data
ye <- curatedMetagenomicData("VogtmannE_2016.relative_abundance", dryrun = F, counts = T, rownames = "long")
ye.se <- ye[[1]]

#Makes the group which is needed to do deAna
grp1 = ifelse(ye.se$study_condition == "control" , 0, 1)
ye.se$GROUP = grp1
ye.se <- ye.se[,!is.na(colData(ye.se)$study_condition)] #removes the data with study_condition of NA 
...
png(filename = "volcanoVogtmannE.png", res = 300, height = 1800, width = 2500)
EnhancedVolcano(rd,
                lab = rownames(rd),
                pCutoff = 0.05,
