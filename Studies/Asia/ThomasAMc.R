pacman::p_load("bugphyzz", "tibble", "curatedMetagenomicData", "EnrichmentBrowser", "SummarizedExperiment", "dplyr", "stringr", "EnhancedVolcano", "dbplyr", "DESeq2", "png", "qusage", "taxize")
source("old_make_signatures.R")
source("deAnaPhyzz.R")

#opens the data
ye <- curatedMetagenomicData("ThomasAM_2019_c.relative_abundance", dryrun = F, counts = T, rownames = "long")
ye.se <- ye[[1]]

#makes the group which is needed for deAna
grp1 = ifelse(ye.se$study_condition == "control" , 0, 1)
ye.se$GROUP = grp1
assay(ye.se) = assay(ye.se) + 1
...
#filters results to only be bacteria of interest
...
#makes volcano plot
png(filename = "volcanoThomasAMc.png", res = 300, height = 1800, width = 2500)
EnhancedVolcano(rd,
                lab = rd$genus,
                pCutoff = 0.05,
